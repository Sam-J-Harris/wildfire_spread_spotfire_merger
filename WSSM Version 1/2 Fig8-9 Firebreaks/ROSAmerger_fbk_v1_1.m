...
function [bigz, bigc, merdata, mcnt, J] = ROSAmerger_fbk_v1_1(bigz,bigc,merdata,mcnt,J,resl,inswt,imswt)
...
if mcnt>imcnt % if fires have merged this step
    bigz = ROSAsmooth_fbk_v1_1(bigz,mcnt,J,resl,inswt,imswt);
end
end
...
if overlaps(poly1,poly2) % determine if the fire lines overlap
    ...
    if J~=2
        z3 = bigz{1}; zdis2 = abs(Zsm-z3(1)*ones(size(Zsm)));
        zdis = zdis+zdis2;
    end
