...
merdata = {}; mcnt=size(bigz,2)-J; tmax=0; rtot=0; % empty array ready for merge data, merge counter (starts at 0), start time at 0, set total runtime initially to zero.
 
%% The TimeStepping Loop
for m=1:steps
    tic; % set timer.
    [bigz,tmax] = ROSAtstep_v1_4(bigz,bigc,tmax,mcnt,J,v0,delta,alpha,beta,lambda,U,tstep,rkswt,pcswt,resl,imswt); % fire time step - see function.
    bigz = ROSAsmooth_v1_4(bigz,mcnt,J,resl,inswt,imswt); % fire line smoothing - see function.
    [bigz, bigc, merdata, mcnt, J] = ROSAmerger_v1_4(bigz,bigc,merdata,mcnt,J,resl,inswt,imswt); % fire merge (and smooth if necessary) - see function.
    ...
    rtime=toc; rtot=rtot+rtime; % update total runtime.
    if m~=1, fprintf(repmat('\b',1,lineLength)), end
    lineLength= fprintf('AAA-LS Fire: Step %d of %d completed in %.1f seconds. \n Total time: %.1f seconds.\n',m,steps,rtime,rtot); % feedback to user on time taken for this time step.
end
end
