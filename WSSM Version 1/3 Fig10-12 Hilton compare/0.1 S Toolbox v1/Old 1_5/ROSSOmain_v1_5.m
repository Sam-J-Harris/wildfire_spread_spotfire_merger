function [Z, RE, mRE] = ROSSOmain_v1_5(N,problem,input,tvec,v0,delta,alpha,beta,lambda,tau,U,s)
% Setup
...
for k=1:tsep
    ...
    rhs = RCArhs(z,theta,sum2,delta,alpha,beta,lambda,U); 
...
for k=1:n
radterm = max(0,v0.*abs(sum2(k))+delta*(real(sum3(k).*conj(sum2(k)))./(abs(sum2(k)))^2));
convterm = max(0,alpha-beta-lambda.*real(sum2(k).*conj(U)));
