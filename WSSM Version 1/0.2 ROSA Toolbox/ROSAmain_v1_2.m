...
function [bigZ, bigC, bigJ, merdata, tmax] = ROSAmain_v1_2(bigz,bigc,J,v0,delta,alpha,beta,lambda,U,tstep,steps,resl,rkswt,pcswt,inswt,imswt)
% ROSA main code
...
merdata = {}; mcnt=size(bigz,2)-J; tmax=0; totaltime=0; % empty array ready for merge data, merge counter (starts at 0), start time at 0, set total(run)time initially to zero.
 
%% The TimeStepping Loop
for m=1:steps
    tic;  % set timer.
    [bigz,tmax] = ROSAtstep_v1_2(bigz,bigc,tmax,merdata,mcnt,J,v0,delta,alpha,beta,lambda,U,tstep,rkswt,pcswt,resl,imswt); % fire time step
