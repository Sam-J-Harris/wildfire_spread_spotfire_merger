...
function [bigz, bigc, merdata, J] = ROSAmerger_v1_1(bigz,bigc,merdata,J,resl,intswt,imswt)
    mswt = 0; % variable to assess if fires have merged this step
    if J~=1
        mergeZ = firemerge(bigz{1}, bigz{2});
        if size(mergeZ,2)==1
            premerge = bigz; % original z data (two fires)
            bigz{1} = mergeZ{1}; J=1; mswt=1; % merged fire
        ...
        bigz{1} = fireintersect(bigz{1},imswt);
    end

    % Fire Interpolation
    if intswt==1 && mswt==0 %interpolate to keep shapes at same resl#
        smno = 2;
        for j=1:J
...
function mergeZ = firemerge(z1,z2)
    ...
    if overlaps(poly1,poly2) % determine if the fire lines overlap
        ...
        mergeZ{1} = circshift(Zsm,round(Pts/2)); 
    else
