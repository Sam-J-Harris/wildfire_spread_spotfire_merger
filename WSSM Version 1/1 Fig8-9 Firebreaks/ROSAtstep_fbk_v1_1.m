...
function [bigz,tmax] = ROSAtstep_fbk_v1_1(bigz,bigc,tmax,merdata,mcnt,J,v0,delta,alpha,beta,lambda,U,tstep,rkswt,pcswt,resl,imswt,shswt)
% = calculates normal velocity step delta z, then timesteps to find z_{t+1}.
% Code:
k1 = firestep(bigz,bigc,J,v0,delta,alpha,beta,lambda,U,pcswt,imswt,shswt); % finding delta Z
[bigz, tmax] = fireRK(k1,bigz,bigc,tmax,merdata,mcnt,J,v0,delta,alpha,beta,lambda,U,tstep,rkswt,pcswt,resl,2,imswt,shswt); % computes RK0, RK2 or RK4 timestepping
end
...
for j=1:J % find delta Z for each fire line
    bigDel{j}=(tau(bigz{j},shswt).*(alpha.*v0 - delta.*curv{j} + max(0, (1-alpha).*v0 + beta.*Pwind{j}+lambda.*cdot(U,nVec{j})))).*nVec{j};
end  
end
...
function [bigz, tmax] = fireRK(k1,bigz,bigc,tmax,merdata,mcnt,J,v0,delta,alpha,beta,lambda,U,tstep,rkswt,pcswt,resl,inswt,imswt,shswt)
...
if rkswt==0 % RK1
    ...
    elseif sint==0 % if fire line self intersected, do RK1 with modified tstep
        ...
        bigz1 = ROSAsmooth_fbk_v1_1(bigz1t,mcnt,J,resl,inswt,imswt); % fire line smoothing
        ...
        if rkswt ==2 %RK2
            ...
            if olap==0 || sint==0, bigz = zrk2; % if a merge has happened, just do RK2
                ...
                bigz2 = ROSAsmooth_fbk_v1_1(bigz2t,mcnt,J,resl,inswt,imswt); % fire line smoothing
                ...
                if olap==0 || sint==0, bigz = zrk2; % if a merge has happened, just do RK2
                    ...
                    bigz3 = ROSAsmooth_fbk_v1_1(bigz3t,mcnt,J,resl,inswt,imswt); % fire line smoothing
